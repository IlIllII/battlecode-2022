...
public strictfp class RobotPlayer {
    ...
    static void runArchon(RobotController rc) throws GameActionException {
        ...
        if (turnCount < 75 && rc.canBuildRobot(RobotType.MINER, dir)) {
    ...
    static void runBuilder(RobotController rc) throws GameActionException {
        ...
        yTarget = (0xFF & archonCoord);
        System.out.println("archonCoord: " + archonCoord + ", xTarget: " + xTarget + ", yTarget: " + yTarget);
        MapLocation moveTarget = new MapLocation(xTarget, yTarget);
        ...
        if (repairing) {
            ...
            for (int i = 1; i < directions.length; i += 2) {
                if (rc.canBuildRobot(RobotType.WATCHTOWER, directions[i])) {
                    MapLocation potentialLoc = rc.adjacentLocation(directions[i]);
                    MapLocation north = potentialLoc.add(directions[0]);
                    MapLocation east = potentialLoc.add(directions[2]);
                    MapLocation south = potentialLoc.add(directions[4]);
                    MapLocation west = potentialLoc.add(directions[6]);

                    if ((!rc.canSenseRobotAtLocation(north) || rc.senseRobotAtLocation(north).mode != RobotMode.TURRET && rc.senseRobotAtLocation(north).mode != RobotMode.PROTOTYPE)
                    && (!rc.canSenseRobotAtLocation(east) || rc.senseRobotAtLocation(east).mode != RobotMode.TURRET && rc.senseRobotAtLocation(east).mode != RobotMode.PROTOTYPE)
                    && (!rc.canSenseRobotAtLocation(south) || rc.senseRobotAtLocation(south).mode != RobotMode.TURRET && rc.senseRobotAtLocation(south).mode != RobotMode.PROTOTYPE)
                    && (!rc.canSenseRobotAtLocation(west) || rc.senseRobotAtLocation(west).mode != RobotMode.TURRET && rc.senseRobotAtLocation(west).mode != RobotMode.PROTOTYPE)) {
        ...
        Direction dir = me.directionTo(moveTarget);
        rc.setIndicatorString(moveTarget.toString());
