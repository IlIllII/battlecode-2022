...
strictfp class SoldierStrategy {
    ...
    static void run(RobotController rc) throws GameActionException {  
        ...
        if (rc.senseNearbyRobots(2, rc.getTeam()).length > 4) {
            Direction dirr = AdvancedMove.getBestDir(rc, primaryTarget);
            if (rc.canMove(dirr)) {
                rc.move(dirr);
            }
        }
        ...
        if (rc.isActionReady() && rc.isMovementReady()) {
            ...
            Direction bestDir = AdvancedMove.getBestDir(rc, tertiaryTarget);
        ...
        if (!rc.isActionReady() && rc.isMovementReady()) {
            Direction retreatDirection = AdvancedMove.getBestDir(rc, repairLocation);
            if (rc.canMove(retreatDirection)) {
                rc.move(retreatDirection);
            }
        }
