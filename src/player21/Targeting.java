...
public class Targeting {
    ...
    static MapLocation getTargetFromGlobalAndLocalEnemyLocations(RobotController rc, RobotInfo[] nearbyEnemies, MapLocation backupTarget) throws GameActionException {
        ...
        for (EnemyLocation globalEnemy : globalEnemyLocations) {
            ...
            if (globalEnemy == null || foundGlobalTarget == true || !globalEnemy.exists) {
                break;
            }
            ...
            // if (me.distanceSquaredTo(globalEnemy.location) <= rc.getType().visionRadiusSquared) {
            //     for (RobotInfo localEnemy : nearbyEnemies) {
            //             if (globalEnemy.location.equals(localEnemy.location)) {
            ...
            if (!foundLocalTarget && !foundGlobalTarget && !clearedGlobalEnemyFromArray) {
        ...
        for (RobotInfo localEnemy : nearbyEnemies) {
            if (!foundGlobalTarget) {
                ...
                || localEnemy.type.equals(RobotType.WATCHTOWER)) {
                    if (localEnemy.health < lowestHPDangerousEnemy && me.distanceSquaredTo(localEnemy.location) <= rc.getType().actionRadiusSquared) {
                        target = localEnemy.location;
        ...
        if (target != null && foundLocalTarget && !foundGlobalTarget) {
