...
public class Comms {
    ...
    */
    ...
    static final int ENEMY_ARRAY_CAPACITY = 64 - 10;

    static int[] enemyIntegerArray = new int[64 - 10];
    ...
    static EnemyLocation getOffensiveLocation(RobotController rc) throws GameActionException {
        ...
        return new EnemyLocation(location, exists);
    }
    ...
    static MapLocation[] getEnemyLocations(RobotController rc) throws GameActionException {
        boolean filling = true;
        int endIndex = 100; // TODO: Placeholder

        for (int i = 0; i < ENEMY_ARRAY_CAPACITY; i++) {
            int bitvector = rc.readSharedArray(ENEMY_TARGETS_INDEX + i);
            
            if (filling && bitvector == 0) {
                filling = false;
                endIndex = i;
            }

            if (filling) {
                enemyIntegerArray[i] = bitvector;
            } else {
                enemyIntegerArray[i] = 0;
            }
        }

        MapLocation[] enemyLocations = new MapLocation[endIndex];
        for (int i = 0; i < endIndex; i++) {
            int bitvector = enemyIntegerArray[i];
            enemyLocations[i] = decodeMapLocationFromBitvector(bitvector);
        }
        
        enemyArrayInsertionIndex = endIndex + ENEMY_TARGETS_INDEX;
        
        return enemyLocations;
    }

    static void setEnemyLocation(RobotController rc, MapLocation location) {
        int bitvector = 0;
        int x = location.x;
        int y = location.y;
        bitvector |= (x << 6) | y;
        assert enemyArrayInsertionIndex < 64;
        try {
            rc.writeSharedArray(enemyArrayInsertionIndex, bitvector);
        } catch (GameActionException e) {
            System.out.println("Tried to setEnemyLocation on an index outside of shared array bounds. Check your indices for off by ones.");
            e.printStackTrace();
        }
    }
...
class EnemyLocation {
    ...
    public EnemyLocation(MapLocation location, boolean exists) {
        this.location = location;
        this.exists = exists;
    }
}